					...
					for (let j = 0; j < mList.length; j++) {
						...
						for (let k = 0; k < t_days[j]; k++) {
							...
							if (dayscNow < daysCount) {
								...
								var subject = this.isHoliday(yearList[i], monthAndDay) || this.isWorkday(yearList[i],
									monthAndDay) || '';
	...
	.day {
		...
		.beginTip {
			...
			&::after {
				...
				border-top: 6rpx solid rgba($color: #000000, $alpha: 0.6);
				/*  #endif  */
				...
				border-top: 10rpx solid rgba($color: #000000, $alpha: 0.6);
				/*  #endif  */
			}
		}
